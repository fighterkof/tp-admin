...
class wf extends Admin
{
    public function initialize()
    {
        ...
        $this->uid = session('auth_id');
        $this->role = session('roleid');
    }
    ...
    public function wfindex($map = [])
    {
        ...
        $this->table = Db :: query("select replace(TABLE_NAME,'".config('database.prefix')."','')as name,TABLE_COMMENT as title from information_schema.tables where table_schema='".config('database.database')."' and table_type='base table' and TABLE_COMMENT <> '';");
